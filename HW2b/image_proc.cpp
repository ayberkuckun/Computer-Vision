...
void image_proc::filtering(Mat image)
{
    resize(image, image, Size(720, 720));
    imshow("original", image);
    waitKey(0);

    Rect Rec(260, 100, 190, 480);
    rectangle(image, Rec, Scalar(255), 1, 8, 0);
    imshow("rectangled", image);
    waitKey(0);
    ...
    Laplacian(Roi, h_pass, 3);

    Rect WhereRec(260, 100, Roi.cols, Roi.rows);
    h_pass.copyTo(blurred(WhereRec));
